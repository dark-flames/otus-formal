...
ctxEqSym (CEqExt Γ≡Δ Γ⊢A Δ⊢B Γ⊢A≡B) = let Δ≡Γ = ctxEqSym Γ≡Δ
    in let Δ⊢A≡B = tyEqStability Γ≡Δ Γ⊢A≡B
    in CEqExt Δ≡Γ Δ⊢B Γ⊢A (TyEqSym Δ⊢A≡B)


ctxEqTrans : ⊢ Γ ≡ⱼ Δ → ⊢ Δ ≡ⱼ Ξ → ⊢ Γ ≡ⱼ Ξ
ctxEqTrans (CEqRefl ⊢Γ) Γ≡Ξ = Γ≡Ξ
ctxEqTrans Γ≡Ξ (CEqRefl ⊢Ξ) = Γ≡Ξ
ctxEqTrans (CEqExt Γ≡Δ Γ⊢A Δ⊢B Γ⊢A≡B) (CEqExt Δ≡Ξ _ Ξ⊢C Δ⊢B≡C) = let Γ≡Ξ = ctxEqTrans Γ≡Δ Δ≡Ξ
    in let Γ⊢B≡C = tyEqStability (ctxEqSym Γ≡Δ) Δ⊢B≡C
    in let Γ⊢A≡C = TyEqTrans Γ⊢A≡B Γ⊢B≡C
    in CEqExt Γ≡Ξ Γ⊢A Ξ⊢C Γ⊢A≡C
...
substDomainStability Γ≡Ξ (SbId ⊢Γ) = let ⊢Ξ = proj₂ (ctxEqWf Γ≡Ξ)
    in SbConv (SbId ⊢Ξ) (ctxEqSym Γ≡Ξ)
substDomainStability (CEqExt Γ≡Ξ Γ⊢A Ξ⊢B Γ⊢A≡B) (SbDropˢ Γ⇒Δ _) = let Ξ⇒Δ = substDomainStability Γ≡Ξ Γ⇒Δ
    in SbDropˢ Ξ⇒Δ  Ξ⊢B
...
tmStability (CEqExt Γ≡Δ Γ⊢A Δ⊢B Γ⊢A≡B) (TmVar _) = let Δ,B⇒Δ = displayMap Δ⊢B
    in let Δ⊢A≡B' = TyEqSubst (TyEqSym (tyEqStability Γ≡Δ Γ⊢A≡B)) (SbEqRefl Δ,B⇒Δ)
    in TmTyConv (TmVar Δ⊢B) Δ⊢A≡B'
...
---- Substitution
-- displayMap : Γ ⊢ A → Γ , A ⊢ drop 1 ⇒ Γ
displayMap Γ⊢A = SbDropˢ (SbId (tyWfCtx Γ⊢A)) (Γ⊢A)
