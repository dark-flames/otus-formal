...
module Otus.Syntax.Typed.Properties.Utils.Substitution where
...
substEqExt₁ : Γ ⊢ γ₁ ≡ⱼ γ₂ ⇒ Δ → Δ ⊢ A → Γ ⊢ a ∷ A [ γ₁ ]ₑ
    → Γ ⊢ γ₁ ▶ a ≡ⱼ γ₂ ▶ a  ⇒ Δ ▷ A
substEqExt₁ Γ⊢γ₁≡γ₂⇒Δ Δ⊢A Γ⊢a∷A[γ₁] = SbEqExt Γ⊢γ₁≡γ₂⇒Δ Δ⊢A (TmEqRefl Γ⊢a∷A[γ₁])

substEqExt₂ : Γ ⊢ γ ⇒ Δ → Δ ⊢ A → Γ ⊢ a ≡ⱼ b ∷ A [ γ ]ₑ 
    → Γ ⊢ γ ▶ a ≡ⱼ γ ▶ b  ⇒ Δ ▷ A
substEqExt₂ Γ⊢γ⇒Δ Δ⊢A Γ⊢a≡b∷A[γ₁] = SbEqExt (SbEqRefl Γ⊢γ⇒Δ) Δ⊢A Γ⊢a≡b∷A[γ₁]

substEqDrop : Γ ⊢ drop x ⇒ Δ → x ≡ y → Γ ⊢ drop x ≡ⱼ drop y ⇒ Δ
substEqDrop Γ⊢dropX⇒Δ refl = SbEqRefl Γ⊢dropX⇒Δ
